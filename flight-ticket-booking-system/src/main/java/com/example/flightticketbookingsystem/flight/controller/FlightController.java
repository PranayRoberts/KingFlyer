...
public class FlightController {
    ...
    public ResponseEntity<List<Flight>> getAllFlight(){
        URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/users/get_all").toUriString());
        return ResponseEntity.created(uri).body(flightService.getAllFlight());
    }

    public ResponseEntity<Flight> getFight(Integer id){
        URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/users/get_flight").toUriString());
